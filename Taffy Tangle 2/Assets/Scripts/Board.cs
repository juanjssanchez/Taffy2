...
public class Board : MonoBehaviour
{
    ...
    private bool MatchesAt(int column, int row, GameObject piece){
        if (column > 1){
            ...
            if (allDots[column - 1, row].GetComponent<Dot>().tag == piece.GetComponent<Dot>().tag && allDots[column - 2, row].GetComponent<Dot>().tag == piece.GetComponent<Dot>().tag){
                return true;
            }
        }
        if (row > 1){
            //if the pieces below me (already generated) are both of the same type as me then ...
            if (allDots[column, row - 1].GetComponent<Dot>().tag == piece.GetComponent<Dot>().tag && allDots[column, row - 2].GetComponent<Dot>().tag == piece.GetComponent<Dot>().tag){
                return true;
            }
        }
